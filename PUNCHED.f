C
C  PROGRAM TO CALCULATE PUNCH SHEAR AND SNAP-THROUGH LOADS FOR EMPERICAL DESIFG REINFORCED CONCRETE BRIDGE DECKS
C
C     `
  ...
  102 END DO
C
C  IU=0, STRESS IN KSI ; IU=1, STRESS IN MPA
C
C  THE 'FCTF' STRESS IN 'PSI' USED IN K&N MODEL
C
      ICOUNT=0
      DELTA=0.
      ASI=0.
  ...
  104 END DO
      WRITE(80,*) ' DELTA   ASI     Y      R4       W   ALPHA        P
     1   EPS    PFAIL     STRN   AB        SK'
      WRITE(90,1000) DELTA,ASI,Y,R4,W,ALPHA,P,EPS,PFAIL,STRN,AB,SK
C     WRITE(80,1000) DELTA,ASI,Y,R4,W,ALPHA,P,EPS,PFAIL,STRN,AB,SK,
      IPFAIL=0
      DELTA=0.
      Y=D/100.
      FACT3=.5*(C-B)
    ...
    2 CONTINUE
      ...
      SIGMA1=P/A
C
C     RITCHART FAILURE CRITERIA FOR 3-D STRESSES IN CONCRETE
C
      SIGMA3C=FPC*(1.+CK*SIGMA1/FPC)
C
C      ADINA FAILURE CRITERIA FOR 3-D STRESSES IN CONCRETE
C
C      SIGMA3C=FPC*(1.+1.5*SIGMA1)
C      IF(IU.EQ.1) SIGMA1=.145*(P/A)
C      IF(IU.EQ.1) SIGMA3C=FPC*(1.+1.5*SIGMA1)
    ...
    3 ESTH=Y*ASI/(.5*B+Y)
      ...
      PFAILP=0.
      WRITE(80,1000) DELTAP,ASIP,YP,R4P,WP,ALPHAP,PP,EPSP,PFAILP,STRNP
     1,EDIA,AREA,SK
      WRITE(90,1000) DELTAP,ASIP,YP,R4P,WP,ALPHAP,PP,EPSP,PFAILP,STRNP
     1,EDIA,AB,SK
  ...
  100 CONTINUE
      ...
      IF(ICOUNT.GT.1) WRITE(80,*)'*******  Punch Load-Yield Strain
     1  in Strap  *******'
      IF(ICOUNT.GT.1) STOP
 1000 FORMAT(1X,F5.2,1X,F5.2,1X,F5.2,2F8.0,F8.2,1X,F8.0,1X,F8.5,2(1X,F8.
     *6),2(1X,F8.2),1X,F8.5,1X,F8.5)
      DELTAP=DELTA
      ASIP=ASI
      YP=Y
      R4P=R4
